...
- 本地模板库：UserDefaults `dietcloud.favoriteFoods.v1`（默认**空**，无内置示例）
- 字段：`id` / `name` / `meal` / `grams` / 热量与宏量（含 fiber）/ `note`
- 首页「常吃」条：点模板 → `create` 到当前 `selectedDateKey`，使用模板默认餐次
- **不**复制照片、`sourceId`、`createdAt`；**不**调用 `update`
- 管理 sheet：新增 / 编辑 / 删除模板；编辑模板**不影响**已有饮食记录
- 食物行左滑 / 长按「加入常吃」：复制名称、餐次、份量、营养、备注
- 快捷添加成功后刷新当日列表与营养总览；历史日写入该日，不跳转日期
- `isMutating` 防重复点击；非法模板校验不调 Repository
- **未做**：云端同步、自定义单位增量（`+50g`）、照片库、食物搜索

### 阶段 15 — 饮食照片库

- 入口：首页工具栏「照片」→ `PhotoLibraryView`（`navigationDestination`）
- 范围：近 7 天 / 近 30 天 / 全部（含今天；全部最多 **100** 张）
- 数据：`fetchBetween` / `fetchAll` 取 `food_items`；按 `photoPaths` 展平为网格项
- 排序：`dateKey` 倒序，同日 `createdAt` 倒序；分区展示
- 签名：与 metadata 解耦；部分 path 失败保留成功图；整批失败仍保留条目占位
- 大图 sheet：名称、餐次、日期、热量/蛋白/碳水/脂肪/纤维、备注；单 path 重试仅 `signedURLs`
- **只读**：不 create/update/delete/upload、不 AI、不改 schema/Storage
- **未做**：分页、编辑/删除照片、按月筛选、瀑布流密度

### 阶段 16 — 身体截图 AI 识别

- 身体 sheet：PhotosPicker → `ImageCompressor` → 内存 data URL → `POST /api/analyze-body`
- **不**上传 Storage、**不**落库截图、**不** log base64/token
- AI **只预填**草稿；用户点保存才 `upsert`；保存日 = 当前 `selectedDateKey`
- 可预填：体重、体脂、BMI、肌肉量、骨量、水分、BMR、内脏脂肪；「更多指标」折叠
- null 不覆盖已有草稿；AI `notes` 仅展示，不覆盖用户备注
- `confidence < 0.5` 警告；AI 日期不一致只提示，不跳日
- 取消 / 关闭 / 保存成功后清除截图 Data
- **未做**：分段四肢 UI、多图、自动改日记日、相机连拍

### 阶段 17 — 提醒路由、相机与饮食照片编辑

- 本地通知 `userInfo` 写入 `dietcloud.reminder.kind`；`UNUserNotificationCenterDelegate` 处理点击
- 路由：早/午/晚 → 今天 + 新增对应餐次；称重 → 今天 + 身体表单；总结 → 今天首页
- 未登录 pending 一次消费；不新增业务 URL scheme；Magic Link 不受影响
- 新增饮食：相册 + 相机（`NSCameraUsageDescription`）；不可用时中文提示
- 编辑主图：未改保留 paths；换图上传单张替换（多图提示）；删除 `photoPaths=[]`
- 回滚：upload 成功但 create/update 失败 → best-effort 删新 path；旧图仅无引用时清理
- **未做**：多图编辑、编辑态 AI、通知 badge、远程推送

### 阶段 18 — 发布前必要检查

- 补充 `PrivacyInfo.xcprivacy`（邮箱/用户 ID/健康/照片/饮食内容；UserDefaults CA92.1；不追踪）
- 补充 `Assets.xcassets/AppIcon`（1024 营销图标）
- `ITSAppUsesNonExemptEncryption = false`（仅 HTTPS）
- `ASSETCATALOG_COMPILER_APPICON_NAME = AppIcon`
- Secrets.xcconfig 仍 gitignore；仓库无 DEVELOPMENT_TEAM
- **需本机**：Apple Developer Team、App Store Connect 创建 App、真机/Archive 签名、确认 Secrets 为生产公开配置
